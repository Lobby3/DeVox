...
import { deployments } from "../src/addresses/deployed";
...
async function main() {
  const [chainId] = await logDeployStart("Baal HO factories");
  ...
  const baalSummonerAddress =
    deployments[0]["v103"][networkName[chainId]].addresses.factory;
  console.log("Summoner Address:", baalSummonerAddress);
  const BaalAdvTokenSummoner = await ethers.getContractFactory(
    "BaalAdvTokenSummoner"
  );

  // deploy proxy upgrades
  const baalSummoner = await upgrades.deployProxy(BaalAdvTokenSummoner);
