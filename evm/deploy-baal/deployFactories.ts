...
import { logDeployStart } from "./logUtils";
...
async function main() {
  await logDeployStart("Baal factories");
  ...
  const baalSummoner = await upgrades.deployProxy(BaalSummoner);
  await baalSummoner.deployed();
  console.log("Factory Contract Address:", baalSummoner.address);
  console.log(
    "imp:",
    await upgrades.erc1967.getImplementationAddress(baalSummoner.address)
  );
  ...
  if (_addresses.DAO == "0x0000000000000000000000000000000000000000") {
    ...
    console.log(
      "transffering ownership of vault summoner too: ",
      _addresses.DAO
    );
    await baalAndVaultSummoner.transferOwnership(_addresses.DAO);
  }

  const vaulTx = baalSummoner.deployTransaction;
  console.log("Transaction Hash:", vaulTx.hash);
  console.log("Block Number:", vaulTx.blockNumber);
}
